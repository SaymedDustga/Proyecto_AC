...
RegisterFile::RegisterFile(sc_module_name moduleName) : sc_module(moduleName), clk("clk"), rwIn("rwIn"), raIn("raIn"), rbIn("rbIn"), wIn("wIn"), weIn("weIn"), aOut("aOut"), bOut("bOut"), and1("and1")
{
	for (int i = 0; i < 32; i++)
	{
		registers[i].clk(clk);
...
void RegisterFile::readA()
{
	aOut.write(regSg[raIn.read()]);
}

void RegisterFile::readB()
{
